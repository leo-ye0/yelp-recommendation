    ...
    class User:
        ...
        def user_feature(row):
            ...
            if row["friends"] != "None":
                ...
                row['friends_cnt'] = 0
            return row
    user_rdd = raw_user_data.map(json.loads).map(User.user_feature).map(lambda row: (row['user_id'], (float(row['average_stars']), np.log1p(row['review_count']), row['elite_cnt'], row['yelping_time'],row["useful"], 
        ...
        def business_feature(row, city_mapping, state_mapping):
            ...
            if latitude is not None:
                try:
                    ...
                    row['latitude'] = 0.0  # Default value or handle as needed
            else:
                row['latitude'] = 0.0
            ...
            if longitude is not None:
                try:
                    ...
                    row['longitude'] = 0.0  # Default value or handle as needed
            else:
                row['longitude'] = 0.0

            return row
    business_rdd = raw_business_data.map(json.loads).map(lambda row: Business.business_feature(row, city_mapping, state_mapping)).map(lambda row: (row['business_id'], (float(row['stars']), np.log1p(row['review_count']), row['attributes_cnt'], row['categories_cnt'], row['city_encoded'], row['state_encoded'], row['is_open'], row['latitude'], row['longitude']))).cache().collectAsMap()
